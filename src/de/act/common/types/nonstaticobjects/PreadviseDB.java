package de.act.common.types.nonstaticobjects;

import java.io.Serializable;
import java.util.Set;

import de.act.central.types.FormularInformation;

public class PreadviseDB implements Serializable{

	private static final long serialVersionUID = 7292334176455252256L;
	private Integer a_preadvise_id;
	private String f_id;
	private Integer acceptance;
	private Boolean loaded;
	private Set<Preadvise_users> preadviseUsers;
	
	public Integer getAcceptance() {
		return acceptance;
	}
	public void setAcceptance(Integer acceptance) {
		this.acceptance = acceptance;
	}
	public void setA_preadvise_id(Integer a_preadvise_id) {
		this.a_preadvise_id = a_preadvise_id;
	}
	public Integer getA_preadvise_id() {
		return a_preadvise_id;
	}
	public void setF_id(String f_id) {
		this.f_id = f_id;
	}
	public String getF_id() {
		return f_id;
	}
	public void setLoaded(Boolean loaded) {
		this.loaded = loaded;
	}
	public Boolean getLoaded() {
		return loaded;
	}
	public Set<Preadvise_users> getPreadviseUsers()
	{
		return preadviseUsers;
	}
	public void setPreadviseUsers(Set<Preadvise_users> preadviseUsers)
	{
		this.preadviseUsers = preadviseUsers;
	}
	
	
}
